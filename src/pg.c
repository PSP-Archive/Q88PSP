...
void print_dec(int x,int y,unsigned int dec)
{
...
//        if(dec=0 && a==0) break;
    ...
    pgPrint_drawbg(x,y,0xffff,0,last);//白で表示
}
...
void readpad(void)
{
	...
	// kmg
	// Analog pad state
//	if (paddata.analog[CTRL_ANALOG_Y] >= 0xff) paddata.buttons=CTRL_A_DOWN;  // DOWN
//	if (paddata.analog[CTRL_ANALOG_Y] <= 0x00) paddata.buttons=CTRL_A_UP;    // UP
//	if (paddata.analog[CTRL_ANALOG_X] <= 0x00) paddata.buttons=CTRL_A_LEFT;  // LEFT
//	if (paddata.analog[CTRL_ANALOG_X] >= 0xff) paddata.buttons=CTRL_A_RIGHT; // RIGHT
...
void readpad_menu(void)
{
	...
	if(paddata.buttons == KeyBuf){
		if ((paddata.frame - KeyTic) > 0x40000) {
			now_pad =  KeyBuf;
			return;
		}
		now_pad = 0;
	...
	KeyBuf  = paddata.buttons;
	KeyTic = paddata.frame;
	now_pad =  KeyBuf;
	return;
