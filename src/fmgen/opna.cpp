...
bool OPNA::LoadRhythmSample(const char* path)
{
	...
	for (i=0; i<6; i++)
	{
		...
		uint32 fsize;
		char buf[16];
		sprintf( buf, "2608_%s.WAV", rhythmname[i] );
		file = osd_fopen_rom( buf, NULL );
...
printf("%04x %04x %04x %04x \n",
	...
	if( verbose_proc ){
	  if( i== 6 ) printf("(Loading Rhythm file OK)..");
	  else        printf("(Loading Rhythm file FAILED)..");
	}
#endif	// forQUASI88
	if (i != 6)
	{
